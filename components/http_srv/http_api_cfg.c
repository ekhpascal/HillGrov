...
esp_err_t h_config_get(httpd_req_t *req) {
    ...
    if (zone == 0) {
         ...
         * is missing or unwrappable, zeroes the struct and says so through
         * hw_present. Taking that signal is what stops the zeroed struct being
         * mistaken for real hardware -- decisively so on the PUT path below.
         * Here the document is still exported: the read-only hardware fields
         * render as zeros for the few seconds before the plane is pulled, which
         * is cosmetic, and refusing the whole config page over it would be
         * worse. */
        if (!hw_present) ESP_LOGW(TAG, "zone %ld: hw plane not cached yet -- exporting a zeroed hw section", zone);
