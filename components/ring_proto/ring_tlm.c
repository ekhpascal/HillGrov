...
_Static_assert(sizeof(hg_hb_t) > 0, "hg_hb_t defined");
_Static_assert(sizeof(hg_ts_t) > 0, "hg_ts_t defined");
_Static_assert(sizeof(hg_assign_t) > 0, "hg_assign_t defined");
_Static_assert(sizeof(hg_fwu_t) > 0, "hg_fwu_t defined");
_Static_assert(sizeof(hg_ack_t) > 0, "hg_ack_t defined");
...
int hg_fwu_pack(const hg_fwu_t *f, uint8_t *out, size_t cap) {
    ...
    /* Validate field lengths (NUL-terminated, so strlen) */
    size_t ssid_len = strlen(f->ssid);
    size_t pass_len = strlen(f->pass);
