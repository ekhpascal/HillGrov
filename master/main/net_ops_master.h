 ...
 * time_svc_apply_mcfg, eight explicit unlocks), and the two have already
 * diverged: 100 ms vs mcfg_ops_edit()'s 6000 ms lock budget, no per-op commit
 * log, and its own inline -1/-2 -> HTTP mapping. Both are correct in lock
 * scope and mapping today. Consolidating it was deliberately NOT done in this
 * branch (it touches a working, bench-verified web path, and this is the last
 * commit before the branch lands); mcfg_ops_edit() is the canonical one, and a
 * third caller -- the panel UI -- must copy THAT, not cfg_put_zone0(). */
