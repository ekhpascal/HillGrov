...
void app_main(void) {
     ...
     * drivers_ok, in that order. wifi_mgr_start()/fw_srv_start() failures
    ...
    if (!ap_ok) ESP_LOGE(TAG, "wifi_mgr_start failed -- AP/STA/fleet-OTA unavailable this boot");
     ...
     * Wi-Fi that is merely absent must never roll a good image back. */
