...
| `GET /api/config?zone=N[&secrets=0]` / `PUT /api/config?zone=N` | §4.6 JSON. GET exports cfg+hw from the master cache (zone 0 = the master's own `mcfg`, which carries no `gen`; `secrets=0` omits passwords). PUT applies the `cfg` plane only — HW is read-only from the web (system spec §4.4): any `hw` key in the body is never merged, only reported as a `warnings` entry, one per leaf. Merge into scratch, `hg_cfg_validate`, first bad `cfg` value → 400 `{error:"INVALID_FIELD"|"VALIDATION", path}`, then `node_mgr_cfg_set` (gen bump + source stamp in the tick, push). 409 while a push for that zone is in flight |
