...
Whole frame validated before relay (cut-through would propagate corruption). Zone routing: own src → drop (loop guard); own dst → consume; broadcast → consume and forward; 0xFE → consume if MAC matches else forward; else ttl−1, re-CRC, forward. *Bench ruling 2026-09-03 (SP3 Task 16):* an **unassigned** node (id 0xFE) identifies its own frames by MAC only — the own-src and own-dst id rules do not apply while unassigned (otherwise the first unassigned hop swallows every ASSIGN_ID and the last one drops every other fresh zone’s heartbeats); the 0xFE MAC-match block is the sole handler of 0xFE-addressed frames, and the ASSIGN_ID handler re-verifies the MAC. Loop protection for 0xFE-sourced frames = the master as ring sink + TTL. **The Master is the ring sink and never forwards** — its returned broadcast is the ring-closed probe. `ring_rx` never runs application code: consumed frames are copied by value into the command task's queue; forwarding happens in place. No TX task — every sender writes one complete encoded frame via `uart_write_bytes()` (driver `tx_mux` serialises; TX ring 1024 B).
...
HEARTBEAT every 2000 ms from the zone control task (plus immediately at link start, after ASSIGN_ID, and on any FAULT/CRITICAL edge, rate-limited 1/500 ms). The heartbeat sequence number comes from a counter of its own (not the zone's shared frame counter) so the Master's gap metric measures heartbeat loss and nothing else; the Master re-learns its baseline whenever a zone's reported uptime regresses (reboot). TIME_SYNC broadcast every 2000 ms is simultaneously: time distribution (zones `settimeofday()` in the control tick when |Δ| > 2 s), the Master-alive signal, zone 1's upstream probe, the ring-closure probe (its return yields `ring_size`), per-zone reachability proof (`online_mask`), and the fleet inhibit channel (`inhibit_mask`, aged out by zones after 600 s).

Master node health: ONLINE → DEGRADED after 5 s without HB (or 3 command timeouts) → OFFLINE after 10 s (alarm, commands refused); UPDATING during fleet OTA (alarms suppressed 180 s); a node whose heartbeat carries a non-zero fault mask is additionally shown FAULT — health state and fault state are orthogonal. With an empty zone table the ring reports IDLE and the open-ring alarm is suppressed (a Master-only installation needs no loopback). Ring OPEN when the Master's own TIME_SYNC stops returning for 5 s; blame names the segment between **D** = the most *upstream* **witness** — a used node that is ONLINE/DEGRADED, not UPDATING, and up for ≥ 10 s (`hb.uptime_s`; a just-booted or never-heard zone reports “no Master seen yet” and is not evidence) — and **U** = the most *downstream* OFFLINE node that has been heard at least once (a row the Master has never heard from is a phantom, not a suspect). Witness order is by measured hops, so D is the largest and U the smallest. If that top witness still reports `master_alive` (`link_flags` b1, the flag behind its own `W_LINK_LOST`), the break lies *above* it among nodes that have not finished falling OFFLINE: the evidence is **unripe** and blame is `no node reports a fault` — the same answer as no evidence at all. Otherwise: `wire M->Z<d>` when U is the Master (nothing offline ⇒ nothing dead), else `Z<u> dead or wire Z<u>->Z<d|M>` — e.g. `NOTIFY RING OPEN Z2 dead or wire Z2->Z3`. With no witness at all D is the Master, its own RX leg closing the segment — but only when nothing alive stands between U and that RX: a zone that is mid-OTA or still inside its boot grace cannot testify yet is on the cable, so while one sits downstream of U the verdict stays `no node reports a fault`. *Bench ruling 2026-09-09 (real wire pulls):* a cut on the leg into node N starves N **and everything downstream of it** of Master frames while they stay ONLINE (their heartbeats still reach the Master's RX), and takes everything **upstream** of the cut OFFLINE (their heartbeats cannot pass it) though they still hear the Master; a dead node is a cut on both its legs and so surfaces as U itself. Two breaks at once ⇒ the one nearest the Master's RX is named; fix it and the next surfaces. The verdict is re-derived on every 1 s health tick, and a **changed** verdict is adopted (and re-announced) only after repeating on 3 consecutive ticks — the OK→OPEN alarm itself is never delayed, but corrections dwell, which drops the flicker of a recovering ring. `nmgr_health_cb` clears the `NTF_RING` rate-limit latch before each RING line so neither a correction nor the `CLOSED` can be swallowed. Ordering falls back to id order only for a node whose hops have never been measured, since enrolment order is not physical order. *Why the first implementation named the wrong cable in all three pulls:* blame was computed **once, at the OK→OPEN edge** — one second before the zones' 6 s link timers could report master-silent, so that branch was always empty and the verdict fell through to weaker ones — and those branches named the leg **upstream** of the silent node, the wrong direction. `upstream_alive` (b0) was never read during the pulls; it was stamped from the zone's own consume queue, which made it a duplicate of b1, and it has since been re-based on the link layer (`ring_link_last_rx_ms`: every validated arrival, forwards included) so it now means “the cable into me is carrying traffic”. Blame does not use it yet. Zone link state: LOST after 6000 ms without a Master-sourced frame (informational; the safety LOCAL transition is §3).
...
*SP3 rulings (2026-09-02, design doc `2026-09-02-sp3-ring-design.md`):* the fleet-OTA serving side ships **in SP3** as a minimal Master AP (fixed `HillGrow`/`hillgrow1`, 192.168.7.7) plus a one-endpoint `fw_srv` streaming `/fw/zone.bin` from the `zone_fw` partition; the image reaches `zone_fw` over USB (`tools/flash_app.py --app zonefw`) until SP4's upload page. Rescue leaves a **trial breadcrumb** after a successful pull: NVS `hg`/`trial` (u8 = the consumed handover's `expect_link`), read-and-erased by the new app to select fleet vs bench trial criteria (§3.10).
...
Only when the slot is `PENDING_VERIFY`. Criteria are firmware-attributable (config parsed or defaulted; I²C bus created, ring driver installed, ADC unit created — device probes deliberately excluded so a bench board can't roll back a good image; `ctrl` ticked ≥ 50× with TWDT happy; heap > 40 KB; Master additionally: AP netif at 192.168.7.7 + httpd up). Handover `expect_link=1` (fleet update) additionally requires a valid Master frame within 180 s; the app learns `expect_link` from the rescue-written breadcrumb (NVS `hg`/`trial`, consumed on first read — §2.9). No breadcrumb = bench criteria. Pass → `esp_ota_mark_app_valid_cancel_rollback()`; fail/timeout → rollback + `W_ROLLBACK` latched on the old image. Bench: `SET OTA CONFIRM`.

### 3.11 Master global safety (details in SP5 feature spec)

Refill valve: **manual, operator-supervised** (duration-bounded `SET OUT`/web button; the operator stops at full — owner decision 2026-09-01, no float switches). The class caps still bound it (max run 300 s, cooldown, daily budget), and a strain-gauge level above max while the valve is on forces it off with a latched fault. Level source: load cell + HX711 (tare/full calibration via CLI/web); low threshold → `inhibit_mask |= PUMPS` fleet-wide; implausible or stuck weight → `F_LEVEL_INVALID` → valve blocked. Ventilation: per-damper position model (CLOSED/OPENING/OPEN/CLOSING/UNKNOWN via end switches or travel time); fan path interlock every tick. `SET SAFE ON` → `inhibit_mask = ALL` (fleet dark ≤ 2 s, self-releasing after 600 s if the Master dies). **Overall grow light** (owner addition 2026-09-01): a room-wide supplemental light on a PCF8575 relay, class LIGHT_MAIN — photoperiod schedule of its own, daily-hours budget, duration-bounded manual override (`SET OUT`/web), included in `SET SAFE ON` and in blackout-shutter coordination. Damper/reservoir hardware variants deferred to SP5 (decision, brainstorm).
...
ERR vocabulary (one token, superset of HillBT): syntax `EMPTY UNKNOWN_CMD BAD_ARGS OUT_OF_RANGE TOO_LONG`; persistence `NVS_OPEN NVS_READ NVS_WRITE CRC_FAIL CFG_VERSION INVALID_FIELD RESTART_REQUIRED MISSING_CHUNK`; gates `AUTH_FAILED LOCKED NOT_LOCAL MASTER_ONLY ZONE_ONLY`; addressing `WRONG_ZONE ZONE_UNKNOWN ZONE_UNASSIGNED ZONE_OFFLINE ZONE_TIMEOUT RING_DOWN ID_CONFLICT`; safety `SAFE_STATE FAULT_ACTIVE INTERLOCK LIMIT BUDGET NOT_CONFIGURED NO_SHELF NO_DEVICE`; misc `BUSY NOT_READY NOT_IMPLEMENTED INTERNAL FW_BUSY FW_NO_IMAGE STORAGE`. `STORAGE` (SP4 Task 8) is the persistence counterpart of `INVALID`: the value was accepted by validation but could not be written (NVS failure, commit-mutex timeout, hash function unavailable) -- the operator should retry or check the flash, not retype the value.
...
`POST /api/cmd` text/plain line → reply verbatim (200 `OK…`, 422 `ERR…`, 413 too long, 503 busy); `GET /api/help`; `GET /api/state` JSON snapshot from the model mirror (built outside the lock); `/api/config` per §4.6; fw endpoints per §1.4. HTTP session is permanently locked (`UNLOCK` rows → `ERR LOCKED`) and session rows → `ERR NOT_LOCAL`. Auth (SP4 decision 2026-09-10, supersedes “AP password only”): a server-side web login — salted SHA-256 password (default = the AP password until changed), `POST /api/login` → `HttpOnly` cookie token, 4 sessions × 30 days persisted in NVS, 5 failures → 60 s lockout; every `/api/*` route except login requires it; plain HTTP (no TLS). Daily access is over the house Wi-Fi (AP+STA; STA address shown on console/AP page; mDNS for desktops); the AP is the fallback. Full route list and rationale: `2026-09-10-sp4-web-ui-design.md`.

*SP4 implementation notes (2026-09-14, binding):* web login is a server-side session — `POST /api/login` with the web password sets an `hg_sess` cookie (4 sessions × 30 days, 5 failed logins → 60 s lockout, constant-time compares); every `/api/*` route except login requires it, while `GET /`, the assets and `GET /fw/zone.bin` do not. Sessions expire on the wall clock only once time quality ≥ SET; before that they live until reboot, and a lockout minted on the frozen clock is dropped when the clock arrives. Every handler that refuses a request with its body unread returns `ESP_FAIL` so httpd closes the socket instead of purging the body (a `Content-Length` lie could otherwise pin the single httpd task); refused uploads drain at most 10 s; the fleet transfer abandons after 120 s. IDF's httpd dispatches a handler only after the first body byte, so a silent client parks the task for `recv_wait_timeout` (5 s, bounded; `lru_purge_enable` displaces the oldest of the 4 sockets). The softAP runs with 802.11w (PMF) disabled: IDF 6 always advertised it and Windows/Intel clients disassociated every ~5 s; the trade is management-frame spoofing resistance within radio range. `SET FW ZONE` and the browser uploads are mutually exclusive.
...
RAM: zone free heap ≈ 220 KB after boot; Master ≈ 95–110 KB at SP1 (Wi-Fi buffer counts and `max_open_sockets 4` must not grow unmeasured; `GET STATUS` reports minimum free heap). *SP4 ruling 2026-09-14:* with httpd + mDNS + cJSON + SNTP + APSTA running, the master's minimum free heap is ≥ 64 KB (measured 87 KB fresh, 75 KB after a master OTA + zone-image upload + fleet pull); the HTTP layer refuses writes below 40 KB (`LOW_HEAP`). Revisit at the P4 migration. Zone NVS wear ≥ 1000 years worst case.
...
| 4 | Master web UI | `wifi_mgr`/`http_srv`/`hg_json`/`hg_fs`/`alarm_mgr`/`history` + pages (gzipped `EMBED_FILES` assets, vanilla JS) | Clients (owner decision 2026-09-09): Chrome on a desktop PC **and** Chrome on an Android phone (portrait, ≈360 px wide) over the master AP — one responsive page set, no app install, no CDN/internet dependency (everything served from the master). Phone on the house Wi-Fi (STA address) or on the AP at 192.168.7.7, behind the web login: dashboard of all zones/shelves, config editing, alarms, both firmware uploads, zone fleet update button. **Done 2026-09-18** — owner acceptance on real Android Chrome (portrait) over the master AP and on desktop Chrome: login, dashboard of both zones, config editing, the bad-value 400 path, alarms carrying the correct ring blame line (`Z1 dead or wire Z1->M`), web-password change and restore, and a zone fleet OTA end-to-end (`FW 2 TRIAL PASS`, zone config surviving the reflash); phone and desktop hold concurrent sessions; heap-min 68 488 B against the §6.4 ≥64 KB bar, web bundle 29 131/65 536 B gz |
| 4b | Master web UI — history | `hg_fs`/`history` + plots, text export, alarm-log persistence, profiles UI | Deferred by decision 2026-09-10 to land **after SP2**, so every series is validated against real telemetry rather than zeros: history store, plots + TSV export in the page set, alarm log surviving reboot, profiles UI (§4.7) |
...
*Revised 2026-09-03 (with the §11.10 platform decision):* audio playback moves to a dedicated **ESP32-DevKitC streamer node** — the only family member with Bluetooth Classic (A2DP sink). The streamer owns the PCM5102A DAC and plays either a BT stream or program audio delivered by the Master over I²S (P4 = I²S master TX → streamer = slave RX; BCLK/WS/DOUT + GND, no MCLK), controlled over a dedicated UART (source, volume, DSP). P4 pins reserved in `docs/pin-mapping.md` (IO46–50). The streamer is a **separate future project**; nothing beyond the pin reservation is specified here. *USB-OTG alternative investigated 2026-09-03:* a USB BT (HCI) dongle on the P4 is NOT viable (IDF has no USB HCI transport for Bluedroid); viable-but-conditional: a BT-receiver dongle presenting as a UAC capture device via the usb_host_uac component (P4 OTG-HS host, verified SOC_USB_OTG_SUPPORTED), or P4 as a UAC device fed from a PC. Bench spike when the kit arrives decides; the ESP32 streamer stays the baseline and the IO46–50 reservation stands.
...
### 11.10 Master platform: ESP32-P4-WIFI6-Touch-LCD-7B (owner decision 2026-09-03)

The Master moves from ESP32-DevKitC-V4 to the Waveshare **ESP32-P4-WIFI6-Touch-LCD-7B** (ESP32-P4, 55 GPIO, 6 UARTs, in-package PSRAM; Wi-Fi 6 via onboard ESP32-C6 over SDIO / `esp_wifi_remote` + `esp_hosted` managed components; integrated 7" 1024×600 MIPI-DSI touch). The kit absorbs: the display node (§11.1 — the separate display MCU + UART API link are RETIRED; the UI runs on the Master, LVGL, as its own late sub-project), the DS3231 (onboard battery-backed RTC — chip id at bring-up), the SC16IS752 + RS-485 module (§11.7 — onboard transceiver on a native UART, 120 Ω jumpers; NOT isolated — keep the external isolated module as an option on a spare UART if the PAR run warrants it), the SD SPI module (onboard SDMMC slot), and the I²S DAC (see the revised §11.3). Verified against IDF 6.0.1: full esp32p4 support incl. the custom-bootloader override and `CONFIG_BOOTLOADER_CUSTOM_RESERVE_RTC` (SOC_RTC_FAST_MEM_SUPPORTED); bootloader offset 0x2000 (flash tools gain per-chip offsets); native USB-Serial-JTAG makes USB the Master's recovery path (a Master rescue image becomes optional; the ZONE fleet-OTA/rescue strategy is unaffected). **Verified on real hardware 2026-09-22 (owner present) — supersedes the earlier "gate test #1" placeholder.** The real HillGrow master (not a spike) runs on the ESP32-P4, flashed via `flash_all.py --target esp32p4`, image VALID: `GET ID` → `OK ID MASTER e8:f6:0a:e0:b6:b0 0 master`. Boot log confirms chip revision v1.3, the partition table matching `master/partitions_p4.csv` exactly, SDIO 4-bit 40000 kHz CLK=18 CMD=19 D0=14 D1=15 D2=16 D3=17 RESET=54, slave chip id 0x0d (esp32c6), SDIO host operating in STREAMING MODE, and **`esp-hosted fw versions: host=3.0.7 coprocessor=3.0.7 (match)`** with `cp_ota` logging "already matches the host -- no OTA". Bootloader 0x60f0 (53% free); app 0xce890 (60% free of the 2 MB smallest app partition). **heap-min 33,909,336 B (~32 MB)** against the §6.4 ≥64 KB bar — vs 68,488 B on the ESP32 master. **The SP4 web suite passes 37/37 unchanged**, including a master self-OTA over Wi-Fi through the C6 (upload → `{"ok":true,"slot":"ota_1"}` → reboot → reconnect ≤90 s → `ota_1 PENDING` → `ota_1 VALID (trial pass)`); a zone image pushed at the master slot was correctly refused `422 IMAGE_MISMATCH`. Ring with two zones: `OK RING OK SIZE 2 ONLINE 0x0006`, both ONLINE, **config generations preserved across the platform transplant** (Z1 gen 3, Z2 gen 183 at the time) because §4.4 reconciliation PULLED each zone's config rather than pushing defaults. Fleet OTA verified on both zones — zone 2's verbatim sequence: `FW 0 ZONE 2 UPDATING` → zone got `192.168.7.10` from the C6's DHCP → `BOOT 2 0.1.0 SW` → `FW 0 ZONE 2 DONE 0.1.0` → `NODE 2 ONLINE` → `FW 2 TRIAL PASS`, **14 s end to end** vs ~35 s on the ESP32 master. Blame verified: holding the `hops == 0` zone in reset gave `RING 0 OPEN Z2 dead or wire Z2->M` after the documented DEGRADED-5 s / OFFLINE-10 s ladder, with clean recovery. After a platform transplant, a master self-OTA, two zone fleet reflashes and a forced ring outage, **both zones' configs were byte-identical to the pre-flash baseline**. **P4 silicon is rev v1.3; IDF treats rev <3.0 and ≥3.0 as mutually exclusive (`CONFIG_ESP32P4_SELECTS_REV_LESS_V3`), so spare boards must be the same revision** — a rev 3.x board is a separate build, not a config tweak. Sequencing: SP3 executes on the DevKitC bench (protocol + zone side are platform-independent); a dedicated master-on-P4 migration sub-project follows when the kit arrives; SP4 web UI lands on the P4. Zones remain ESP32-DevKitC-V4 — the no-Wi-Fi-in-zone rule, ring framing and rescue flow are untouched.

**Product-level gap surfaced by the migration: the P4 `factory` partition is empty, removing the ESP32 master's rescue safety net.** The ESP32 master kept a rescue app in `factory`, so two bad OTA slots still left a recovery path; on the P4, recovery currently means USB and a PC. Observed behaviour (three times, bench-verified 2026-09-22): a ≥10 s rescue-button hold logs `hg_boot: button held 10000 ms -> factory`, then `image at 0x30000 has invalid magic byte`, `Factory app partition is not bootable`, and the bootloader **falls through gracefully to an OTA slot** — it does not brick and does not reboot-loop, and NVS survives that path. This raises a design question the migration has not yet answered: the rescue app's own recovery mechanism is Wi-Fi (`rescue_wifi.c` / `rescue_http.c`), which on the P4 runs through `esp_hosted` and the C6 co-processor — so rescue on the P4 would depend on the co-processor being functional, and if the C6 is what is broken, the recovery path is broken too. That coupling did not exist on the ESP32, where the radio shared the die. This needs a deliberate design decision (possibly a non-Wi-Fi route via the C6-UART header or the microSD slot), not a straight retarget of the existing rescue app.
