...
Master node health: ONLINE → DEGRADED after 5 s without HB (or 3 command timeouts) → OFFLINE after 10 s (alarm, commands refused); UPDATING during fleet OTA (alarms suppressed 180 s); a node whose heartbeat carries a non-zero fault mask is additionally shown FAULT — health state and fault state are orthogonal. With an empty zone table the ring reports IDLE and the open-ring alarm is suppressed (a Master-only installation needs no loopback). Ring OPEN when the Master's own TIME_SYNC stops returning for 5 s; blame names the segment between **U** = the most *downstream* OFFLINE node (smallest hops, the Master if none) and **D** = the most *upstream* ONLINE/DEGRADED node reporting master-silent (`master_alive` clear, largest hops, the Master if none) — `NOTIFY RING OPEN Z2 dead or wire Z2->Z3`, or `wire M->Z2` when U is the Master (nothing offline ⇒ nothing dead), or `ring open (no node reports a fault)` when neither end exists. *Bench ruling 2026-09-09 (real wire pulls):* a cut on the leg into node N starves N **and everything downstream of it** of Master frames while they stay ONLINE (their heartbeats still reach the Master's RX), and takes everything **upstream** of the cut OFFLINE (their heartbeats cannot pass it) though they still hear the Master; a dead node is a cut on both its legs and so surfaces as U itself. Blame reads `master_alive` (b1) only — `upstream_alive` (b0) stayed set on nodes whose upstream leg had just been pulled, and the earlier "smallest-hops node with `upstream_alive` clear, else lowest-id silent node" rule named the wrong cable in all three pulls. Ordering is by measured hops (counted at the Master's RX, so 0 is the node feeding it and the first hop after the Master's TX is highest); id order is the fallback only for a node whose hops have never been measured, since enrolment order is not physical order. UPDATING zones are ignored at both ends. The verdict is re-derived on every health tick while the ring is open — at the 5 s mark nothing has reached OFFLINE (10 s) and no starved zone has had its 6 s of silence plus a heartbeat to report the loss, so blame starts vague and sharpens within ~10 s; a changed verdict emits another `RING OPEN <blame>`, an unchanged one stays silent. The `wire M->Z<d>` reading asserts that every zone above D is still delivering heartbeats, so it is withheld (blame stays `ring open (no node reports a fault)`) while any zone's heartbeats have stopped without having reached OFFLINE yet — that zone is about to become U, and naming the Master's TX leg meanwhile would point at a cable that is not in the path. Zone link state: LOST after 6000 ms without a Master-sourced frame (informational; the safety LOCAL transition is §3).
