...
| 4 | Master web UI | `wifi_mgr`/`http_srv`/`hg_json`/`hg_fs`/`alarm_mgr`/`history` + pages (gzipped `EMBED_FILES` assets, vanilla JS) | Clients (owner decision 2026-09-09): Chrome on a desktop PC **and** Chrome on an Android phone (portrait, ≈360 px wide) over the master AP — one responsive page set, no app install, no CDN/internet dependency (everything served from the master). Phone on the AP at 192.168.7.7: dashboard of all zones/shelves, config editing, alarms, history plots + text export, both firmware uploads, zone fleet update button |
