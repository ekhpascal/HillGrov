...
5. **`CMD_SRC_RING`** is added to `cmd_session_t`'s source enum. Ring-executed commands: `CMDF_SESSION` rows answer `NOT_LOCAL` (they act on a session that doesn't meaningfully exist), everything else per its gates; the ring session has no unlock (DEBUG ENABLE over the ring per §5.8 operates the *target zone's* ring-session unlock state).
