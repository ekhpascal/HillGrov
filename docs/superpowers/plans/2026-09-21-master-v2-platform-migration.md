...
2. `esp_partition_find_first(ESP_PARTITION_TYPE_DATA, 0x41, "cp_fw")`; return -1 if absent. Then read the **16-byte `HGFW` header** at offset 0 and return -1 unless the magic matches, the length is non-zero and within the partition, and the CRC32 over the image body checks out. This is the SAME staging format `zone_fw` already uses — see `build_zonefw_image()` in `tools/flash_app.py` and `validate_image()` in `components/fw_srv/fw_srv.c`. It is not decoration: `cp_fw` is a **data** partition, so the image length cannot be recovered by parsing `esp_image` segments (that is bootloader territory) and has to be carried explicitly. Verifying the CRC *before* the push also means a truncated or corrupt staged image is caught while the C6 is still running working firmware — the alternative is a half-written radio, and the only recovery for that is the board's C6-UART header.
3. `eh_host_cp_ota_begin()`, then `esp_partition_read` + `eh_host_cp_ota_write` in **1536-byte chunks** (`EH_RPC_OTA_CHUNK_MAX`) for exactly the header's declared length, starting at offset 16 — **not** the whole partition. Then `eh_host_cp_ota_end()`. Pushing the full 0x180000 partition would send ~40% padding, add ~4 s to a transfer already sized against an 8 s task watchdog, and hand the radio trailing `0xFF`.
...
Call `cp_ota_sync()` in `app_main.c` after `wifi_mgr_start()` **and after `ring_link_start()` / `node_mgr_start()`**. A working AP proves the RPC path the OTA needs, so it cannot go first — but it must not go ahead of the ring either. `eh_host_cp_ota_begin()` alone has a 30 s timeout (`EH_HOST_OTA_BEGIN_TIMEOUT_MS`, because the co-processor erases its whole OTA slot), and `eh_host_mcu_transport_get_fw_version()` returns 0 both for a factory C6 **and for a dead SDIO link** — so a staged image plus an unresponsive co-processor would otherwise delay the ring by up to 30 s on every single boot. The ring is the master's core function; the radio is not. Gate the whole attempt on the CP link actually being up, and log the outcome at WARN.

Never auto-reboot the master after a successful push. The update is version-gated, so if the new image reports the expected version there is nothing to repeat — but if it reports something else, a reboot would loop forever. Log loudly and carry on instead.

Add `--app cpfw` to `tools/flash_app.py`, mirroring the existing `zonefw` case in BOTH respects: it gets its own branch that writes no `otadata` (`cp_fw` is a data partition, not an app slot), and it stages the image behind the same 16-byte `HGFW` header that `build_zonefw_image()` writes — magic, little-endian length, CRC32, reserved word — ahead of the raw image. Reuse that helper rather than writing a second one. Take the raw CP binary path as an explicit `--cp-image` argument, since the `eh_cp` project that produces it is not in this repo yet (Task 9 promotes it). Write the result into `cp_fw` at `0xBB0000`, and refuse an image that will not fit once the header is added. **Use the `binascii.crc32` seed-0 convention**, the same check-value family `hg_blob.c`'s `hg_crc32(0, ...)` computes — NOT `hg_otadata.py`'s `0xFFFFFFFF`-seeded `esp_rom_crc32_le`, which is a different family for a different, bootloader-owned structure. `build_zonefw_image()`'s own docstring warns against conflating the two. Document in its `--help` that the source is the `eh_cp` project's `eh_cp_wifi_softap.bin`, and that building the CP needs `CONFIG_EH_TRANSPORT_CP_SDIO_MODE_STREAM=y` because esp_hosted's default SW_AGGR mode requires an ESP-IDF patch that 6.0.1 lacks — **do not run `eh.py patch-idf` on the shared IDF install.**
