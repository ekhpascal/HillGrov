...
**A dead node cannot validate a blame line; only a pulled wire can.** SP3 signed off break blame against a zone held in reset — the one break a single pair of hands can make — and when the owner finally pulled single wires on 2026-09-09 the line was wrong for all three legs: the M→Z2 pull printed `Z1 dead or wire Z1->M`, the Z2→Z1 pull printed `Z2 dead or wire M->Z2`, the Z1→M pull printed `Z1 dead or wire Z2->Z1`. A reset board is a symmetrical break (both of its legs die at once), so it cannot show which *direction* the analyzer is reading; worse, the rule it had been built on — "the smallest-hops node whose `upstream_alive` bit is clear" — never fired at all on the bench, because `upstream_alive` stayed **set** on a node whose upstream leg had just been cut, and blame silently fell through to weaker branches. The physics only becomes visible with a single leg down: a cut starves that leg's node *and everything downstream of it* of master frames while their heartbeats keep arriving, and takes everything *upstream* of it OFFLINE while those nodes still hear the master — so the break is the segment between the most-downstream offline node and the most-upstream master-silent live node (§2.7, now the U/D rule). **Rule:** a diagnostic that names a physical thing is only tested by breaking that physical thing, one at a time, in each direction; an approximation that breaks two things at once (a board in reset, a powered-down node) proves the alarm fires, never that it points the right way. And a link flag earns the name "break detector" only once it has been observed to go dark on a real break.
