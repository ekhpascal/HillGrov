...
**Product implication:** a P4 crash leaves a *live but unserved* HillGrow AP, which is worse than the AP disappearing outright — a phone will happily join and then silently time out, with nothing telling the operator the master is down.

**Fix:** not decided — this is carried forward as an open decision for the Master v2 migration project, not a closed bug. Two directions were on the table in the spike: a host watchdog that resets the co-processor on a hang, or accepting the failure mode and making it legible some other way.
