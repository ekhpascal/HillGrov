...
const cmd_entry_t *zone_table(int *n) {
    if (!s_init) {
        ...
        int n_zone = ZONE_CMD_ROWS_N < remaining ? ZONE_CMD_ROWS_N : remaining;
        int n_trial = remaining - n_zone;
        ...
        memcpy(s_table + n_common + n_zone, OTA_TRIAL_ROWS, (size_t)n_trial * sizeof(cmd_entry_t));
        s_n = total;
        s_init = 1;
    }
    if (n) *n = s_n;
